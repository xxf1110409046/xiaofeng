...
/**
 * Created by Mrper on 2015/5/30. 日历视图控件
 */
public class CalendarView extends LinearLayout {
	...
	private void showCalendar(Calendar calendar) {
		...
		if (firstOfWeek <= 0)
			firstOfWeek = 7 + firstOfWeek;// 处理星期的显示
		...
		// 还原月份数据
		txtTitle.setText(new SimpleDateFormat("yyyy年MM月").format(calendar
				.getTime()));
		// 设置日历显示的标题
	...
	public class CalendarAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (item.dayType == DayType.DAY_TYPE_FORE
					|| item.dayType == DayType.DAY_TYPE_NEXT) {// 标识文字颜色
			...
			if (isDateEqual(Calendar.getInstance(), tmpCalendar)
					&& item.dayType == DayType.DAY_TYPE_NOW) {// 判断是不是今天
				iimg.setVisibility(View.INVISIBLE);
				...
				public void onClick(View v) {
					...
					case DAY_TYPE_NOW:
						if (isDateEquals(Calendar.getInstance(), tmpCalendar)
							...
							Toast.makeText(context, "请选择今天或者今天以前的时间", 1000)
									.show();
						}
						break;
					case DAY_TYPE_NEXT:
						Toast.makeText(context, "请选择今天或者今天以前的时间", 1000).show();
						break;
					}
				}
			};
